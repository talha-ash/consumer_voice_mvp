defmodule ConsumerVoiceMvp.Calls do
  ...
  def update_call_status(call_id, status) do
    ...
    case call do
      nil ->
        {:error, "Call not found against call_id: #{call_id}"}
  ...
  def get_initiated_call({:employee, employee_id, company_id}) do
    ...
    case Repo.one(query) do
      nil ->
        {:error, "Initiated Call not found against employee_id: #{employee_id}"}
  ...
  def get_initiated_call({:client, client_id, company_id}) do
    ...
    case Repo.one(query) do
      nil ->
        {:error, "Initiated Call not found against client_id: #{client_id}"}
  ...
  def get_waiting_call({:employee, employee_id, company_id}) do
    ...
    case Repo.one(query) do
      nil ->
        {:error, "Waiting Call not found against employee_id: #{employee_id}"}
  ...
  def get_waiting_call({:client, client_id, company_id}) do
    ...
    case Repo.one(query) do
      nil ->
        {:error, "Waiting Call not found against client_id: #{client_id}"}
  ...
  def get_active_call(call_id, company_id) do
    query =
      from c in Calls.Call,
        where:
          c.id == ^call_id and c.company_id == ^company_id and c.status == @call_status_active,
        select: c

    case Repo.one(query) do
      nil ->
        {:error, "Active Call not found against call_id: #{call_id}"}
  ...
  def get_initiated_call(call_id, company_id) do
    query =
      from c in Calls.Call,
        where:
          c.id == ^call_id and c.company_id == ^company_id and c.status == @call_status_initiated,
        select: c

    case Repo.one(query) do
      nil ->
        {:error, "Initiated Call not found against call_id: #{call_id}"}
  ...
  def get_waiting_call(call_id, company_id) do
    query =
      from c in Calls.Call,
        where:
          c.id == ^call_id and c.company_id == ^company_id and c.status == @call_status_waiting,
        select: c

    case Repo.one(query) do
      nil ->
        {:error, "Waiting Call not found against call_id: #{call_id}"}
